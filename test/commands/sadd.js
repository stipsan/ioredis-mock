...
describe('sadd', () => {
  it('should add 1 item to set', () => {
    ...
    return redis.sadd('foos', 'bar').then(status => expect(status).toBe(1))
      .then(() => expect(redis.data.get('foos').has('bar')).toBe(true));
  });
  it('should add 2 items to set', () => {
    const redis = new MockRedis();

    return redis.sadd('foos', 'foo', 'baz').then(status => expect(status).toBe(2))
      .then(() => {
  ...
  it('should not increase length when adding duplicates', () => {
    ...
    return redis.sadd('key', 'value', 'value')
