...
describe('getbit', () => {
  it('should return old bit value of key', () => {
    ...
    return redis.setbit('foo', 1, 0).then(result => {
  ...
  it('should padd if offset out of range', () => {
    ...
    return redis
      ...
      .then(() => {
        return expect(redis.data.get('foo')).toBe('\x00@')
      })
  })
  ...
  it('should ovveride bit value of key', () => {
    ...
    return redis
      ...
      .then(() => {
        return expect(redis.data.get('foo')).toBe('rar')
      })
  })
  ...
  it('should create key if not exist', () => {
    ...
    return redis
      ...
      .then(() => {
        return expect(redis.data.get('foo')).toBe('@')
      })
  })
