...
describe('bzmpop', () => {
  ...
  ;(process.env.IS_E2E ? it.skip : it)(
    ...
    async () => {
      const res = await redis.bzmpop(2, 'foo', 'bar', 'MAX', 'COUNT', 2)
  ...
  ;(process.env.IS_E2E ? it.skip : it)(
    ...
    async () => {
      const res = await redis.bzmpop(2, 'foo', 'bar', 'MIN', 'COUNT', 2)
  ...
  ;(process.env.IS_E2E ? it.skip : it)(
    ...
    async () => {
      const res = await redis.bzmpop(2, 'foo', 'bar', 'MAX', 'COUNT', 3)
  ...
  ;(process.env.IS_E2E ? it.skip : it)('should return items from the second key if the second is empty', async () => {
    const res = await redis.bzmpop(2, 'bar', 'foo', 'MIN', 'COUNT', 3)
    expect(res).toEqual(['foo', [['first', 1], ['second', 2], ['third', 3]]])
  })
})
