  ...
  describe(command, () => {
    it('should increment an integer with passed increment in hash', () => {
      ...
      return redis[command]('highscores', 'user:1', 100)
        .then(result => {
          return expect(result).toBe(9100)
        })
        .then(() => {
          return expect(redis.data.get('highscores')['user:1']).toBe('9100')
        })
    })
    it('should create hash if not exists', () => {
      ...
      return redis[command]('stats', 'hits', 100)
        ...
        .then(() => {
          return expect(redis.data.get('stats').hits).toBe('100')
        })
    })
    ...
    it('should decrement value in hash if negative integer is passed', () => {
      ...
      return redis[command]('highscores', 'user:1', -100)
        .then(result => {
          return expect(result).toBe(8900)
        })
        .then(() => {
          return expect(redis.data.get('highscores')['user:1']).toBe('8900')
        })
    })
  })
})
