...
runTwinSuite('lpos', command => {
  describe(command, () => {
    ...
    it('should support COUNT option', async () => {
      ...
      expect(await redis[command]('mylist', 'a', 'COUNT', 10)).toEqual([0, 2, 4])
    })
    ...
    it('should support COUNT with RANK option', async () => {
      ...
      expect(await redis[command]('mylist', 'a', 'RANK', 2, 'COUNT', 2)).toEqual([2, 4])
      expect(await redis[command]('mylist', 'a', 'RANK', -2, 'COUNT', 2)).toEqual([2, 0])
    })
    ...
    it('should support MAXLEN option', async () => {
      ...
      expect(await redis[command]('mylist', 'a', 'MAXLEN', 3, 'COUNT', 2)).toEqual([0, 2])
    ...
    it('should combine all options correctly', async () => {
      ...
      expect(await redis[command]('mylist', 'a', 'RANK', 2, 'COUNT', 2, 'MAXLEN', 5)).toEqual([2, 4])
    })
    ...
    ;(process.env.IS_E2E ? it.skip : it)(
      ...
      async () => {
        ...
        return redis[command]('mylist', 'a', 'RANK', 0).catch(err =>
          expect(err.message).toBe('ERR RANK can not be zero: use 1 to start from the first match, 2 from the second ... or use negative to start from the end of the list')
        )
      }
    )
