...
runTwinSuite('lmpop', command => {
  describe(command, () => {
    ...
    it('should pop from the first non-empty list using LEFT direction', async () => {
      ...
      expect(
        remaining.map(v => (Buffer.isBuffer(v) ? v.toString() : v))
      ).toEqual(['b', 'c'])
    })
    ...
    it('should throw error for invalid numkeys', async () => {
      await expect(redis[command](0, 'list1', 'LEFT')).rejects.toThrow(
        'ERR numkeys should be greater than 0'
      )

      await expect(redis[command](-1, 'list1', 'LEFT')).rejects.toThrow(
        'ERR numkeys should be greater than 0'
      )

      await expect(redis[command]('invalid', 'list1', 'LEFT')).rejects.toThrow(
        'ERR numkeys should be greater than 0'
      )
    })

    it('should throw error for invalid direction', async () => {
      await expect(redis[command](1, 'list1', 'INVALID')).rejects.toThrow(
        'ERR syntax error'
      )

      await expect(redis[command](1, 'list1', 'UP')).rejects.toThrow(
        'ERR syntax error'
      )
    })
    ...
    it('should throw error for wrong number of arguments', async () => {
      ...
      await expect(redis[command](1, 'list1')).rejects.toThrow(
        "ERR wrong number of arguments for 'lmpop' command"
      )
    })
    ...
    it('should throw error for wrong COUNT syntax', async () => {
      await expect(redis[command](1, 'list1', 'LEFT', 'COUNT')).rejects.toThrow(
        'ERR syntax error'
      )
    ...
    it('should throw error for non-list key', async () => {
      ...
      await expect(redis[command](1, 'string_key', 'LEFT')).rejects.toThrow(
